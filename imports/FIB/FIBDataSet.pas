...
uses
 ...
 pFIBInterfaces,pFIBEventLists, FIBMDTInterface,
  ...
  {$IFNDEF NO_GUI}
   {$IFDEF D_XE2}
   ,Vcl.Forms,Vcl.Controls // IS GUI units
   {$ELSE}
...
//    rdCachedUpdateStatus: TCachedUpdateStatus;// Bit 7 is Calcs
  ...
  private
   ...
   procedure    GetAsDB_KEYProc(var DB_KEY:Ansistring);
   procedure Prepare;
  ...
  protected
    ...
    function  GetAnsiValue(var Value: Ansistring): Boolean;
    function  GetAsString: string; override;
    function  GetAsData: AnsiString;
    procedure SetAsData(const Value: AnsiString);
    {$IFDEF D12+}
...
//    {$ENDIF}
  public
    ...
    property  AsOctetsData:AnsiString read GetAsData write SetAsData;
  published
...
//TNT Controls Interface
  ...
  IWideStringField = interface
    ...
    function GetAsWideString: {$IFDEF D12+}UnicodeString;  {$ELSE} WideString; {$ENDIF}
    procedure SetAsWideString(const Value: {$IFDEF D12+}UnicodeString  {$ELSE} WideString {$ENDIF});
    ...
    //--
    property AsWideString: {$IFDEF D12+}UnicodeString  {$ELSE} WideString {$ENDIF} read GetAsWideString write SetAsWideString{inherited};
   ...
   protected
    ...
    {$ENDIF}
   public
    function GetAsWideString: {$IFDEF D12+}UnicodeString;   {$ELSE} WideString; {$ENDIF} {$IFDEF D10+} override;{$ENDIF}
    procedure SetAsWideString(const aValue:{$IFDEF D12+}  UnicodeString{$ELSE} WideString {$ENDIF});
         {$IFDEF D10+} override; {$ENDIF}
    function GetAsVariant: Variant; override;
    function GetAsString: string;  override;
    procedure SetAsString(const Value: string); override;
    function GetBlobId:TISC_QUAD;
 ...
 {$ENDIF}    
    function  GetAsExtended: Extended;  {$IFDEF D12+} override; {$ENDIF}
    procedure SetAsExtended(Value: Extended); {$IFDEF D12+} override; {$ENDIF}
...
{$ENDIF}
 ...
 {$ENDIF}
  ...
  TDataSetRunStateValue=(drsInCacheRefresh,drsInSort,drsInOpenByTimer,
   ...
   drsGetBlobStream,drsInLoadFromStream,drsInFieldValidate ,drsInFieldAsData
  );
  ...
  TFIBCustomDataSet = class({$IFDEF D12+}TWideDataset{$ELSE}TDataset{$ENDIF},ISQLObject,IFIBDataSet)
  protected
    (*
...
{$UNDEF FIB_INTERFACE}
    ...
    function  IsVisibleStat(Buffer: dbPChar): Boolean;
    function  IsVisible(Buffer: dbPChar): Boolean; virtual;
  ...
  protected
    FValidatingFieldBuffer:PAnsiChar;
...
procedure    TFIBStringField.GetAsDB_KEYProc(var DB_KEY:Ansistring);
begin
 DB_KEY:=GetAsDB_Key
end;
...
begin
  if not FPrepared then
   Prepare;
  if not GetAnsiValue(Result) then
   Result := ''
  else
...
begin
  if not FPrepared then
   Prepare;
  if not GetAnsiValue(FStringBuffer) then
   Result := ''
  else
   if FNeedUnicodeConvert then
   {$IFDEF D12+}
     Result:=UTF8ToString(FStringBuffer)
   {$ELSE}
     Result:=UTF8Decode(FStringBuffer)
   {$ENDIF}
   else
    {$IFDEF SUPPORT_KOI8_CHARSET}
    if (FieldKind=fkData )  and  (DataSet is TFIBDataset) then
...
function  TFIBStringField.GetAsData: AnsiString;
begin
  if Dataset.IsEmpty then
  begin
    Result:=''; Exit;
...
procedure TFIBStringField.SetAsData(const Value: AnsiString);
begin
  if FieldKind<>fkData then

   raise Exception.Create('Method TFIBStringField.SetOctetsData:'#13#10+
     'DataSet '+CmpFullName(Self)+' field "'+FieldName+'" is not data field');
  if CharacterSet<>'OCTETS' then
     raise Exception.Create('Method TFIBStringField.SetOctetsData:'#13#10+
     'DataSet '+CmpFullName(Self)+' field "'+FieldName+'" is not octets field');
  Include(TFIBCustomDataSet(DataSet).FRunState,drsInFieldAsData);
  try
...
function TFIBStringField.GetAnsiValue(var Value: Ansistring): Boolean;
begin
  if FIsDBKEY then
  begin
   GetAsDB_KEYProc(Value) ;
   Result:=Value<>''
  end
  else
  begin
    ...
    if Result then
     if FReservedBuffer^=#0 then
      Value :=''
     else
     begin
      Value:=FReservedBuffer
     end;
  end;
end;
...
function TFIBMemoField.GetAsWideString: {$IFDEF D12+}UnicodeString;{$ELSE} WideString;{$ENDIF}
begin
 with  TFIBDataSet(DataSet).Database do
  if NeedUTFEncodeDDL  and IsUnicodeConnect or (FCharSetID in UnicodeCharSets) then
  {$IFDEF D12+}
...
procedure TFIBMemoField.SetAsWideString(const aValue: {$IFDEF D12+}UnicodeString{$ELSE} WideString{$ENDIF});
begin
 with  TFIBDataSet(DataSet).Database do
  if (NeedUTFEncodeDDL and IsUnicodeConnect) or (FCharSetID in UnicodeCharSets) then
   {$IFDEF D12+}
...
(*      case VarType(KeyValues[i]) of
       varString,varOleStr {$IFDEF D12+},varUString{$ENDIF}:*)
...
function  TFIBCustomDataSet.IsVisibleStat(Buffer: dbPChar): Boolean;
begin
...
function TFIBCustomDataSet.IsVisible(Buffer: dbPChar): Boolean;
begin
...
begin
  ...
  begin
    ...
    ReadRecordCache(PRecordData(Buff)^.rdRecordNumber, dbPChar(Buff), False);
...
begin
 ...
 case t of
  ...
  varOleStr,varString{$IFDEF D12+},varUString{$ENDIF} :
  begin
   Field:=FN(Fld);
   if Field =nil then
          ...
          begin
            ...
            begin
             ...
             if (tf.DataType in [ftWideString,ftString]) then
              FieldSize:=Length(PAnsiChar(Buffer + fi^.fdDataOfs+DiffSizesRecData-1));
             Move(
              PAnsiChar(Buffer + fi^.fdDataOfs+DiffSizesRecData-1)^, PAnsiChar(PIsNull+SizeOf(Boolean))^,
              FieldSize
             );
            end;
          end;
...
begin
 Result := False;
 if (drsInFieldValidate in FRunState) and not (State=dsOldValue) and (Buffer<>nil) then
     ...
     //   Field.Validate(Buffer);
        ...
        sp := False;
         if (Field is TFIBStringField) and (Buffer<>nil) and (PAnsiChar(Buffer)[0] = #0)
          and (TFIBStringField(Field).FEmptyStrToNull and not (drsInCacheRefresh in FRunState))
         then
         begin
