...
{$I FIBPlus.inc}
uses
  Classes, SysUtils,ibase,DB,FIBDatabase,FIBQuery,FIBDataSet,pFIBProps,StrUtil,
...
*)
  ...
  // DBInfo
    ...
    FClientLib:string;
    FCanUseProcParamDomain:boolean;
    FCanUseProcParamRelation:boolean;
    FExistTabRelType:boolean;
    FSQLDialect     :integer;
  //
...
begin
  ...
  Stream.WriteString(DefTerminator+NewLine)
end;
...
begin
  ...
  Stream.WriteString(NewLine + ')');
  case FRelationType of
   4:   Stream.WriteString(NewLine+'ON COMMIT PRESERVE ROWS ');
   5:   Stream.WriteString(NewLine+'ON COMMIT DELETE ROWS ');
  end;
end;
...
var
  ...
  ConStr, Str: string;
  QCharset :TFIBQuery;
  QNames, QFields, QPrimary: TFIBQuery;
  ...
  try
    ...
    try
      ...
      begin
        ...
        QNames.SQL.Text := QRY_EXISTS_TAB_RELATION_TYPE;
        QNames.ExecQuery;
        FExistTabRelType:=Boolean(QNames.Fields[0].AsInteger);
        QNames.Close;

        if FExistTabRelType then
        begin
          if FSysInfos then
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  if 'TABLE1' = Trim(QForeign.Fields[3].AsString) then
                    beep;
                  SetLength(FFields, 1);
...
//          if not  OnlyNames then
      ...
      begin
        QNames.SQL.Text := QRY_EXISTS_PROC_PARAMDOMAINS;
        QNames.ExecQuery;
        FCanUseProcParamDomain:=Boolean(QNames.Fields[0].AsInteger);
        QNames.Close;



        QNames.SQL.Text := QRY_EXISTS_PROC_PARAMRELATION;
        QNames.ExecQuery;
        FCanUseProcParamRelation:=Boolean(QNames.Fields[0].AsInteger);
        QNames.Close;

        FItems[Ord(otProcedure)].Childs.Clear;
        QNames.SQL.Text := QRYProcedures;
        if not FCanUseProcParamDomain then
         QFields.SQL.Text := QRYProcFields
        else
        if not FCanUseProcParamRelation then
        ...
        begin
          ...
          begin
            with TMetaProcedure(CurObject) do
              LoadFromDataBase(QNames, QFields, QCharset, FCanUseProcParamDomain,FCanUseProcParamRelation);
...
// Kind= 0 - standard, =1 -FakeViewTrigger,=2 Alter - View Trigger
  ...
  begin
    ...
    begin
      Stream.WriteString(NewLine+Format('/* %s */', [comment])+NewLine);
      ...
      begin
        ...
        begin
          ...
          Stream.WriteString(Separator+NewLine);
        end;
      end;
    end;
  end;
  ...
  begin
    ...
    begin
      Stream.WriteString(NewLine+NewLine+Format('/* %s */', [comment]));
      ...
      begin
        ...
        if CurObj is TMetaProcedure then
          ...
          CurObj.GenerateObjectDDL(Stream);
...
begin
  ...
  Stream.WriteString('SET SQL DIALECT '+IntToStr(FSQLDialect)+';'+NewLine);
  Stream.WriteString('SET NAMES  '+FConnectCharset+';'+NewLine);
  if dtoUseSetClientLib in Options then
    Stream.WriteString('SET CLIENTLIB  '''+FClientLib+''';'+NewLine+NewLine);
      

  if dtoUseCreateDB in Options then
  begin
   Stream.WriteString('CREATE DATABASE '''+FName+''''+NewLine+
   'USER ''SYSDBA'' PASSWORD ''masterkey'''+NewLine+'PAGE_SIZE '+IntToStr(FPageSize)+NewLine);
   if Length(FDefaultCharset)>0 then
    Stream.WriteString('DEFAULT CHARACTER SET '+FDefaultCharset+DefTerminator+NewLine);
  end
  else
   Stream.WriteString('CONNECT '''+FName+''' USER ''SYSDBA'' PASSWORD ''masterkey'';'+NewLine);
  ...
  begin
    Stream.WriteString(NewLine+'/* PROCEDURES */');
    for I := 0 to ProceduresCount - 1 do
    begin
      Stream.WriteString(NewLine+NewLine);
      Procedures[I].SaveToAlterDDL(Stream);
      Stream.WriteString(vProcTerminator+NewLine);
    end;
  end;
  if dtoUseSetTerm in Options then
   Stream.WriteString('SET TERM ;^'+NewLine+NewLine);
  Stream.WriteString('COMMIT WORK;');
...
const QRY_NAMES:array[TDBObjectType] of string  =
    ...
    MetaNodeClasses:array[TDBObjectType] of TMetaObjectClass =(
     ...
     TMetaUDF, TMetaRole, TCustomMetaTrigger
    );
...
//  if (Result=nil) and ForceLoad then
  ...
  begin
     ...
     begin
      FExistTabRelType:= not
           VarIsNull(FInternalTransaction.DefaultDatabase.QueryValue(QRY_EXISTS_TAB_RELATION_TYPE,0));
      if FExistTabRelType then
     ...
     try
      QNames.Params[0].asString:=SavedName;
      QNames.ExecQuery;
      if not QNames.Eof then
      begin
       if ObjectType in [otDomain,otUDF,otTable,otView,otProcedure] then
       ...
       case ObjectType of
        ...
        otUDF:
         ...
         otProcedure:
         begin
          FCanUseProcParamDomain:= not
           VarIsNull(FInternalTransaction.DefaultDatabase.QueryValue(QRY_EXISTS_PROC_PARAMDOMAINS,0));
          FCanUseProcParamRelation:=      not
           VarIsNull(FInternalTransaction.DefaultDatabase.QueryValue(QRY_EXISTS_PROC_PARAMRELATION,0));
          ...
          TMetaProcedure(Result).LoadFromDataBase(QNames,QFields,QCharset,
           FCanUseProcParamDomain,FCanUseProcParamRelation
          )
         end;
         otException:
...
begin
  ...
  Stream.WriteString(')');
  if not FActive then
    Stream.WriteString(Format('%sALTER INDEX %s INACTIVE', [NewLine, FormatName]));
end;
...
begin
  if Copy(FName, 0, 6) = 'INTEG_' then
    ...
    Stream.WriteString(Format('ALTER TABLE %s ADD CONSTRAINT %s '+NewLine+
     ' FOREIGN KEY (',
      [TMetaTable(FOwner).FormatName, FormatName]));
...
// For Updateble View
  ...
  begin
   Stream.WriteString(Format('CREATE TRIGGER %s FOR %s%s',
    [Name, TCustomMetaObject(FOwner).Name, NewLine]));
  end
  else
  begin
  ...
  Stream.WriteString(Format(' POSITION %d%s%s', [FPosition, NewLine, 'AS BEGIN  POST_EVENT ''FAKE''; END']));
end;
...
begin
  ...
  if FActive then
    Stream.WriteString('ACTIVE ');
  Stream.WriteString(Format(' %s%s', [NewLine, FSource]));
end;
...
begin
  ...
  begin
   Stream.WriteString(Format('CREATE TRIGGER %s FOR %s%s',
    [Name, TCustomMetaObject(FOwner).FormatName, NewLine]));
  end
  else
  begin
   Stream.WriteString(Format('CREATE TRIGGER %s%s',
...
//  Stream.WriteString(Format(' POSITION %d%s', [FPosition, NewLine,FSource]));
  Stream.WriteString(' POSITION '+IntToStr(FPosition)+ NewLine+FSource);
end;
...
begin
  ...
  for I := 0 to FieldsCount - 1 do
  begin
    Stream.WriteString(NewLine + '   ' + Fields[I].FormatName);
    if I <> FieldsCount - 1 then
      Stream.WriteString(',');
  end;
...
begin
  ...
  Stream.WriteString(NewLine + 'AS' + NewLine + 'BEGIN' + NewLine +
    '  SUSPEND;' + NewLine + 'END');
end;
...
begin
  ...
  if FValidationSource<>'' then
   Stream.WriteString(NewLine+ FValidationSource);
end;
...
begin
  ...
  begin
   ...
   FUseDomain:=Copy(FDomain,1,4)<>'RDB$';
...
begin
  ...
  begin
   ...
   begin
    ...
    Stream.WriteString(NewLine+'  '+FUserList[i]);    
   end
   else
     ...
     for j:=i-1 downto 0 do
      ...
      begin
       ...
       if (Length(FUserList[j])mod 100+Length(FUserList[i])mod 100)>100 then
        FUserList[j]:=FUserList[j]+','+NewLine+'  '+FUserList[i]
       else
        FUserList[j]:=FUserList[j]+','+FUserList[i];
       DeleteIndex(i, TDynStrArray(FUserList));
       DeleteIndex(i, TDynStrArray(FFields));
