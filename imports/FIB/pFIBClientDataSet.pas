...
{$I FIBPlus.inc}
uses
  pFIBInterfaces,  Windows, Messages, SysUtils, Classes,
...
{$ENDIF}
     function  GetAsExtended: Extended;  {$IFNDEF D12+} virtual{$ELSE} override {$ENDIF};
     function  GetAsString: string; override;
     function  GetAsInt64: Int64; virtual;
     function  GetAsVariant: Variant; override;
     procedure SetAsFloat(Value: Double); override;

     procedure SetAsExtended(Value: Extended); {$IFNDEF D12+} virtual{$ELSE} override {$ENDIF};
...
begin
 case VarType(Value) of
  ...
  varString,varOleStr{$IFDEF D12+},varUString{$ENDIF}    : AsString :=Value;
